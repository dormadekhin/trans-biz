	...
	<div>
		...
		<jet-form-section @submitted="createApiToken">
			...
			<template #description>
				Токены API позволяют сторонним службам аутентифицироваться в приложении от имени пользователя.
			</template>
			...
			<template #form>
				...
				<div class="col-span-6" v-if="availablePermissions.length > 0">
					<jet-label for="permissions" value="Разрешения"/>
		...
		<div v-if="tokens.length > 0">
			...
			<div class="mt-10 sm:mt-0">
				<jet-action-section class="p-3">
					<template #title>
						Управление токенами API
					</template>

					<template #description>
						Вы можете удалить любой из имеющихся у вас токенов, если они больше не нужны.
					</template>

					<!-- API Token List -->
					<template #content>
						<div class="divide-y">
							<div class="flex items-center justify-between px-4 py-5" v-for="token in tokens"
							     :key="token.id">
								<div>
									{{ token.name }}
								</div>
								...
								<div class="flex items-center">
									<div class="text-sm text-gray-400" v-if="token.last_used_at">
										Последний раз был использован {{ fromNow(token.last_used_at) }}
									</div>
		...
		<jet-dialog-modal :show="displayingToken" @close="displayingToken = false">
			...
			<template #content>
				<div>
					Скопируйте свой новый токен API. В целях безопасности он больше не будет отображаться.
				</div>
		...
		<jet-dialog-modal :show="managingPermissionsFor" @close="managingPermissionsFor = null">
			...
			<template #footer>
				...
				<jet-button class="ml-2" @click.native="updateApiToken"
